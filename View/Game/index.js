...
const generateGameList = (data) => {
  return data
    .map((game) => {
      return `<a href='/game/${game._id}' class='
       shadow-md  shadow-gray-900 
      flex rounded-md relative'>
      <img class='h-60 
      rounded-md w-full' src='${game.bannerImage}' alt='not found'/>
      <div class='absolute z-20 p-4 top-0 left-0 text-white w-full h-full rounded
